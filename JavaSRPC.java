...
public class JavaSRPC
{
		...
		public void run()
		{
			while (socket != null && socket.isConnected())
			{
				...
				try
				{
					...
					System.out.println("received: " + command + "(" + command.ordinal() + ") " + rseqno + " " + rsubport);
					// Data
					switch (command)
					{
						case CONNECT:
	
							sendCommand(Command.CACK, RPCState.CACK_SENT, rsubport, rseqno);
							break;
						...
						case QUERY:
							...
							sendCommand(Command.QACK, RPCState.QACK_SENT, rsubport, rseqno);
							readData(is, fragment);
							respond(rsubport, rseqno);
						...
						case RESPONSE:
							...
							{
								sendCommand(Command.RACK, RPCState.IDLE, null, fragment, fragmentCount, rsubport, rseqno);// subport, seqno);
							}
							break;
						...
						case DISCONNECT:
							...
							sendCommand(Command.DACK, RPCState.TIMEDOUT, subport, seqno);
							break;
						...
						case FRAGMENT:
							...
							if (readData(is, fragment))
							{
								sendCommand(Command.FACK, RPCState.FACK_SENT, null, fragment, fragmentCount, subport, seqno);
							}
							break;
						...
						case PING:
							...
							sendCommand(Command.PACK, state, subport, seqno);
							break;
	...
	{
		IDLE, QACK_SENT, RESPONSE_SENT, CONNECT_SENT, QUERY_SENT, AWAITING_RESPONSE, TIMEDOUT, DISCONNECT_SENT, FRAGMENT_SENT, FACK_RECEIVED, FRAGMENT_RECEIVED, FACK_SENT, SEQNO_SENT, CACK_SENT
	}
			...
			while (true)
			{
						...
						.contains(state))
				{
					...
					if (ticksLeft <= 0)
					{
						...
						if (attemptsLeft > 0)
						{
							System.err.println("Resending " + attemptsLeft);
							try
							{
								resend();
								lastTicks *= 2;
	...
	public String respond(int subport, int seqno) throws IOException{
	
		
		final byte[] queryBytes = ("OK" + "\0").getBytes(CHARSET);
		
		if (socket != null){
			//waitForState(EnumSet.of(RPCState.QACK_SENT, RPCState.IDLE));
			...
			sendCommand(Command.RESPONSE, RPCState.RESPONSE_SENT, bos.toByteArray(), fragmentCount, fragmentCount, subport, seqno);
	...
	public String call(final String query) throws IOException
	{
		if (socket != null)
		{
			...
			sendCommand(Command.QUERY, RPCState.QUERY_SENT, bos.toByteArray(), fragmentCount, fragmentCount, subport, seqno);
			waitForState(EnumSet.of(RPCState.IDLE, RPCState.TIMEDOUT));
	...
	public void connect(final InetAddress address, final int port) throws SocketException, IOException
	{
		...
		System.out.println("my client sub port is " + subport);
		// logger.info("Subport: " + subport);
		this.address = address;
		this.port = port;
		
		sendCommand(Command.CONNECT, RPCState.CONNECT_SENT, "HWDB\0".getBytes(CHARSET), 1, 1, subport, seqno);
	...
	{
		sendCommand(Command.DISCONNECT, RPCState.DISCONNECT_SENT, subport, seqno);
		waitForState(EnumSet.of(RPCState.IDLE, RPCState.TIMEDOUT));
	}
	...
	private byte[] getBytes(final Command command, int sp, int sn) throws IOException
	{
		return getBytes(command, null, (byte) 1, (byte) 1, sp,  sn);
	}

	private byte[] getBytes(final Command command, final byte[] data, final int fragment, final int fragmentCount, final int sp, final int sn)
			throws IOException
	{
		final ByteArrayOutputStream bos = new ByteArrayOutputStream();
		final DataOutputStream os = new DataOutputStream(bos);
	...
	private boolean readData(final DataInputStream is, final int fragment) throws IOException
	{
		...
		if (state == RPCState.QACK_SENT){
			responseData = new ByteArrayOutputStream(dataLength);
			responseData.write(data);
			lastFragment = fragment;
			System.out.println(new String(responseData.toByteArray(), CHARSET));
			return true;
		}
		if (state == RPCState.CACK_SENT){
			responseData = new ByteArrayOutputStream(dataLength);
			responseData.write(data);
			lastFragment = fragment;
			return true;
	...
	private synchronized void sendCommand(final Command command, final RPCState newState, final int sp, final int sn) throws IOException
	{
		// logger.info("Send " + command);
		System.out.println("sending " + command + "(" + command.ordinal() + ") " + sn + " " + sp);
		sendBytes(getBytes(command, sp, sn));
		setState(newState);
	}

	private synchronized void sendCommand(final Command command, final RPCState newState, final byte[] data,
			final int fragment, final int fragmentCount, final int sp, final int sn) throws IOException
	{
		// logger.info("Send " + command);
		sendBytes(getBytes(command, data, fragment, fragmentCount, sp, sn));
		setState(newState);
	}
